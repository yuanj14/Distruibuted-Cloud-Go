...
type Simulator struct {
	...
	// Track which servers have completed each snapshot (snapshotId -> map[serverId]bool)
	snapshotComplete *SyncMap
	// Channel to signal when a snapshot is complete on all servers (snapshotId -> chan bool)
...
func (sim *Simulator) StartSnapshot(serverId string) {
	...
	// TODO: IMPLEMENT ME
	// Initialize tracking for this snapshot
	sim.snapshotComplete.Store(snapshotId, make(map[string]bool))
	sim.snapshotDone.Store(snapshotId, make(chan bool))
	
	// Start the snapshot on the specified server
...
func (sim *Simulator) NotifySnapshotComplete(serverId string, snapshotId int) {
	...
	// TODO: IMPLEMENT ME
	// Mark this server as complete for this snapshot
	completeVal, _ := sim.snapshotComplete.Load(snapshotId)
	complete := completeVal.(map[string]bool)
	complete[serverId] = true
	
	// Check if all servers have completed this snapshot
	if len(complete) == len(sim.servers) {
		// Signal that the snapshot is complete
		doneVal, _ := sim.snapshotDone.Load(snapshotId)
		done := doneVal.(chan bool)
		done <- true
	}
}
...
func (sim *Simulator) CollectSnapshot(snapshotId int) *SnapshotState {
	...
	// TODO: IMPLEMENT ME
	// Wait for all servers to complete the snapshot
	doneVal, _ := sim.snapshotDone.Load(snapshotId)
	done := doneVal.(chan bool)
	<-done
	...
	for _, serverId := range getSortedKeys(sim.servers) {
		server := sim.servers[serverId]
		serverSnapVal, _ := server.snapshots.Load(snapshotId)
		serverSnap := serverSnapVal.(*SnapshotState)
		
		// Merge tokens
		for sid, tokens := range serverSnap.tokens {
			snap.tokens[sid] = tokens
		}
